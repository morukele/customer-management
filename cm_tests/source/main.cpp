...
int main(int argc, char *argv[])
{
    ...
    qDebug() << "Starting thest suite...";
    qDebug() << "Accessing tests from " << &TestSuite::testList(); // this is static thats why we can call it from anywhere in the project with the header.
    qDebug() << TestSuite::testList().size() << " tests detected";

    int failedTestsCount = 0;

    for(auto* i : TestSuite::testList()) {
